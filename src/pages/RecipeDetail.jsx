...
const RecipeDetail = () => {
  ...
  const getRecommendations = async (label) => {
    try {
      const recommend = await fetchRecipes({ query: label, limit: 5 });
  ...
  useEffect(() => {
    if (recipe?.label) {
      getRecommendations(recipe.label);
    }
  }, [recipe]);
        ...
        <div className='w-full flex flex-col md:flex-row gap-8 py-20 pxx-4 md:px-10'>
          ...
          <div className='w-full md:w-2/4 md:border-r border-slate-800 pr-1'>
            <div className='flex flex-col gap-5'>
              ...
              <br />
              <ButtonGroup recipeId={id} recipe={recipe} /> 
            </div>
